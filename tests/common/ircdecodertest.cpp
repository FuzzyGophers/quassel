...
IrcMessage parse(const std::string& message)
{
    ...
    IrcDecoder::parseMessage([](const QByteArray& data) { return QString::fromUtf8(data); },
...
{
    EXPECT_EQ(parse("foo bar baz asdf"), IrcMessage("", "foo", {"bar", "baz", "asdf"}));
}

TEST(IrcDecoderTest, with_source)
{
    EXPECT_EQ(parse(":coolguy foo bar baz asdf"), IrcMessage("coolguy", "foo", {"bar", "baz", "asdf"}));
}

TEST(IrcDecoderTest, with_trailing_param)
{
    EXPECT_EQ(parse("foo bar baz :asdf quux"), IrcMessage("", "foo", {"bar", "baz", "asdf quux"}));
    EXPECT_EQ(parse("foo bar baz :"), IrcMessage("", "foo", {"bar", "baz", ""}));
    EXPECT_EQ(parse("foo bar baz ::asdf"), IrcMessage("", "foo", {"bar", "baz", ":asdf"}));
}

TEST(IrcDecoderTest, with_source_and_trailing_param)
{
    EXPECT_EQ(parse(":coolguy foo bar baz :asdf quux"), IrcMessage("coolguy", "foo", {"bar", "baz", "asdf quux"}));
    EXPECT_EQ(parse(":coolguy foo bar baz :  asdf quux "), IrcMessage("coolguy", "foo", {"bar", "baz", "  asdf quux "}));
    EXPECT_EQ(parse(":coolguy PRIVMSG bar :lol :) "), IrcMessage("coolguy", "PRIVMSG", {"bar", "lol :) "}));
    EXPECT_EQ(parse(":coolguy foo bar baz :  "), IrcMessage("coolguy", "foo", {"bar", "baz", "  "}));
}
...
TEST(IrcDecoderTest, with_tags)
{
    ...
    EXPECT_EQ(parse("@a=b foo"), IrcMessage({{IrcTagKey("a"), "b"}}, "", "foo"));

    EXPECT_EQ(parse("@example.com/a=b foo"), IrcMessage({{IrcTagKey("example.com", "a"), "b"}}, "", "foo"));

    EXPECT_EQ(parse("@example.com/subfolder/to/a=b foo"), IrcMessage({{IrcTagKey("example.com/subfolder/to", "a"), "b"}}, "", "foo"));

    EXPECT_EQ(parse("@v\\/e\\/n\\/d\\/o\\/r/tag=b foo"), IrcMessage({{IrcTagKey("v\\/e\\/n\\/d\\/o\\/r", "tag"), "b"}}, "", "foo"));
}
...
{
    EXPECT_EQ(parse(":src JOIN #chan"), IrcMessage("src", "JOIN", {"#chan"}));
    EXPECT_EQ(parse(":src JOIN :#chan"), IrcMessage("src", "JOIN", {"#chan"}));
}

TEST(IrcDecoderTest, with_and_without_last_param)
{
    EXPECT_EQ(parse(":src AWAY"), IrcMessage("src", "AWAY"));
    EXPECT_EQ(parse(":src AWAY "), IrcMessage("src", "AWAY"));
}

TEST(IrcDecoderTest, tab_is_not_considered_SPACE)
{
    EXPECT_EQ(parse(":cool\tguy foo bar baz"), IrcMessage("cool\tguy", "foo", {"bar", "baz"}));
}

TEST(IrcDecoderTest, with_weird_control_codes_in_the_source)
{
    EXPECT_EQ(parse(":coolguy!ag@net"
                    "\x03"
                    "5w"
                    "\x03"
                    "ork.admin PRIVMSG foo :bar baz"),
              IrcMessage("coolguy!ag@net"
                         "\x03"
                         "5w"
                         "\x03"
                         "ork.admin",
...
TEST(IrcDecoderTest, with_tags_source_and_params)
{
    ...
    EXPECT_EQ(parse("@tag1=value1;tag2;vendor1/tag3=value2;vendor2/tag4 COMMAND param1 param2 :param3 param3"),
              IrcMessage({{IrcTagKey("tag1"), "value1"},
                         ...
                         "COMMAND",
                         {"param1", "param2", "param3 param3"}));
    EXPECT_EQ(parse("COMMAND"), IrcMessage("", "COMMAND"));
...
TEST(IrcDecoderTest, broken_messages_from_unreal)
{
    ...
    EXPECT_EQ(parse(":gravel.mozilla.org MODE #tckk +n "), IrcMessage("gravel.mozilla.org", "MODE", {"#tckk", "+n"}));
    EXPECT_EQ(parse(":services.esper.net MODE #foo-bar +o foobar  "), IrcMessage("services.esper.net", "MODE", {"#foo-bar", "+o", "foobar"}));
}

TEST(IrcDecoderTest, tag_values)
{
    EXPECT_EQ(parse("@tag1=value\\\\ntest COMMAND"), IrcMessage({{IrcTagKey("tag1"), "value\\ntest"}}, "", "COMMAND"));
    EXPECT_EQ(parse("@tag1=value\\1 COMMAND"), IrcMessage({{IrcTagKey("tag1"), "value1"}}, "", "COMMAND"));
    EXPECT_EQ(parse("@tag1=value1\\ COMMAND"), IrcMessage({{IrcTagKey("tag1"), "value1"}}, "", "COMMAND"));
}
