...
bool MetricsServer::startListening()
{
        ...
        if (!addr.setAddress(listen_term)) {
            ...
            case QAbstractSocket::IPv6Protocol:
                if (_v6server.listen(addr, port)) {
                    qInfo() << qPrintable(
                        tr("Listening for metrics requests on IPv6 %1 port %2").arg(addr.toString()).arg(_v6server.serverPort()));
                    success = true;
                }
                else
                    qWarning() << qPrintable(
            ...
            case QAbstractSocket::IPv4Protocol:
                if (_server.listen(addr, port)) {
                    qInfo() << qPrintable(
                        tr("Listening for metrics requests on IPv4 %1 port %2").arg(addr.toString()).arg(_server.serverPort()));
                    success = true;
                }
                else {
    ...
    if (!success) {
        qWarning() << qPrintable(
            tr("Metrics could not open any network interfaces to listen on! No metrics functionality will be available"));
    }
...
void MetricsServer::respond()
{
    ...
    if (requestPath == "/metrics") {
        ...
        for (const auto& key : _sessions.keys()) {
            ...
            socket->write(
                QString("quassel_client_sessions{user=\"%1\"} %2 %3\n").arg(name).arg(_clientSessions.value(key, 0)).arg(timestamp).toUtf8());
            ...
            socket->write(
                QString("quassel_network_sessions{user=\"%1\"} %2 %3\n").arg(name).arg(_networkSessions.value(key, 0)).arg(timestamp).toUtf8());
            ...
            socket->write(
                QString("quassel_message_queue{user=\"%1\"} %2 %3\n").arg(name).arg(_messageQueue.value(key, 0)).arg(timestamp).toUtf8());
            ...
            socket->write(QString("quassel_login_attempts{user=\"%1\",successful=\"false\"} %2 %3\n")
                              ...
                              .arg(timestamp)
                              .toUtf8());
            socket->write(QString("quassel_login_attempts{user=\"%1\",successful=\"true\"} %2 %3\n")
                              .arg(name)
        ...
        if (!_certificateExpires.isNull()) {
            ...
            socket->write(
                QString("quassel_ssl_expire_time_seconds %1 %2\n").arg(_certificateExpires.toMSecsSinceEpoch() / 1000).arg(timestamp).toUtf8());
        }
        socket->close();
    }
